void diffuse(int b, float[] x, float[] x0, float diffusion, float dt)
{
...
void linear_solve(int b, float[] x, float[] x0, float a, float c)
{
  ...
  for(int k = 0; k < ITER_COUNT; k++)
  {
    for(int j = 1; j < N - 1; j++)
    {
      for(int i = 1; i < N - 1; i++)
      {
        // new value = old value + linear interpolation of its neibourght
...
// Motion related with the velocity
