...
{
  public HScrollBar(float value,float min,float max,float x,float y,float len,float incrementFactor,String varName)
  {
    super(value,min,max,x,y,len,incrementFactor,varName);
    setRectX(getValue()*(len/(getMax() - getMin())) - getValue());
  }
